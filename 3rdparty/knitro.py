...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
#+++  KNITRO 9.0.1 Python API
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++


'''Python interface for KNITRO.

This module provides a Python interface for the KNITRO solver.
...
try:
	...
	print "Failed to load the KNITRO native library '" + _knitroLibraryFile + "'."
...
class KTR_array_handler:
	...
	def _cStringArray (pyArray):
		...
		if pyArray != None:
			n = len (pyArray)
			if n > 0:
				return (ctypes.c_char_p * n) (*pyArray)
		return None
...
class _KTR_puts_wrapper (object):
	def __init__ (self, kc, fnPtr):
		self._kc = kc
		self._fnPtr = fnPtr
		self.c_fnPtr = _KTR_puts (self.call)
	def set_KTR_context_ptr (self, kc):
		self._kc = kc
	def call (self, str, userParams):
		return self._fnPtr (str, _getUserParams (self._kc))
...
class _KTR_callback_wrapper (object):
	def __init__ (self, kc, fnPtr):
		self._kc = kc
		self._fnPtr = fnPtr
		self.c_fnPtr = _KTR_callback (self.call)
	def set_KTR_context_ptr (self, kc):
		self._kc = kc
	def call (
		self,
		...
		if evalRequestCode == KTR_RC_EVALFC or evalRequestCode == KTR_RC_EVALGA:
			x = _userArray (n, c_x)
			obj = [c_obj[0]]
			c = _userArray (m, c_c)
			...
			return ret
		elif evalRequestCode == KTR_RC_EVALH or evalRequestCode == KTR_RC_EVALHV or evalRequestCode == KTR_RC_EVALH_NO_F or evalRequestCode == KTR_RC_EVALHV_NO_F:
			x = _userArray (n, c_x)
			...
			return ret
		else: # evalRequestCode == KTR_RC_NEWPOINT, KTR_RC_NODE, or KTR_RC_MSPROCESS
			x = _userArray (n, c_x)
			lambda_ = _userArray (n+m, c_lambda)
			obj = [c_obj[0]]
			c = _userArray (m, c_c)
...
class _KTR_newpt_callback_wrapper (object):
	def __init__ (self, kc, fnPtr):
		self._kc = kc
		self._fnPtr = fnPtr
		self.c_fnPtr = _KTR_newpt_callback (self.call)
	def set_KTR_context_ptr (self, kc):
		self._kc = kc
	def call (
		self,
...
class _KTR_ms_initpt_callback_wrapper (object):
	def __init__ (self, kc, fnPtr):
		self._kc = kc
		self._fnPtr = fnPtr
		self.c_fnPtr = _KTR_ms_initpt_callback (self.call)
	def set_KTR_context_ptr (self, kc):
		self._kc = kc
	def call (
		self,
...
def KTR_new_zlm (fnPtr, userParams, pZLMcontext):
	fnPtrWrapper = _KTR_puts_wrapper (None, fnPtr)
	kc = _knitro.KTR_new_zlm (c_fnPtr, userParams, pZLMcontext)
	if kc:
		fnPtrWrapper.set_KTR_context_ptr (kc)
		_registerCallback (kc, fnPtrWrapper)
		_registerUserParams (kc, userParams)
...
def KTR_load_param_file (kc, filename):
	return _knitro.KTR_load_param_file (kc, filename)
...
def KTR_save_param_file (kc, filename):
	return _knitro.KTR_save_param_file (kc, filename)
...
def KTR_set_int_param_by_name (kc, name, value):
	return _knitro.KTR_set_int_param_by_name (kc, name, value)
...
def KTR_set_char_param_by_name (kc, name, value):
	return _knitro.KTR_set_char_param_by_name (kc, name, value)
...
def KTR_set_double_param_by_name (kc, name, value):
	return _knitro.KTR_set_double_param_by_name (kc, name, ctypes.c_double (value))
...
def KTR_set_char_param (kc, param_id, value):
	return _knitro.KTR_set_char_param (kc, param_id, value)
...
def KTR_get_int_param_by_name (kc, name, value):
	...
	ret = _knitro.KTR_get_int_param_by_name (kc, name, ctypes.byref (c_value))
...
def KTR_get_double_param_by_name (kc, name, value):
	...
	ret = _knitro.KTR_get_double_param_by_name (kc, name, ctypes.byref (c_value))
	value[0] = c_value.value
	return ret
...
def KTR_load_tuner_file (kc, filename):
	return _knitro.KTR_load_tuner_file (kc, filename)
...
def KTR_get_release (size, release):
	_knitro.KTR_get_release (size, release)
...
def KTR_set_feastols (kc, cFeasTols, xFeasTols, ccFeasTols):
	_knitro.KTR_set_feastols (kc, _cDoubleArray (cFeasTols), _cDoubleArray (xFeasTols), _cDoubleArray (ccFeasTols))
...
def KTR_set_names (kc, objName, varNames, conNames):
	_knitro.KTR_set_names (kc, objName, _cStringArray(varNames), _cStringArray(conNames))
...
def KTR_init_problem (
	...
	of the C version. However, C ctypes.POINTERs and arrays must be provided
	as basic Python lists (of length 1 for C ctypes.POINTERs).
...
def KTR_mip_init_problem (
	...
	if (xLoBnds != None and n != len (xLoBnds)) or (xUpBnds != None and n != len (xUpBnds)):
		raise ValueError ("Arrays xLoBnds and xUpBnds have invalid non-zero sizes (must be exactly n)!")
	if xInitial != None and (len (xInitial) > 0 and n != len (xInitial)):
		raise ValueError ("Array xInitial has an invalid non-zero size (must be exactly n)!")
	if (cType != None):
		m = len (cType)
	else:
		m = 0
	if ( (cFnType != None and m != len (cFnType)) or (cFnType == None and m != 0) or
		 (cLoBnds != None and m != len (cLoBnds)) or (cLoBnds == None and m != 0) or
		 (cUpBnds != None and m != len (cUpBnds)) or (cUpBnds == None and m != 0) ):
		raise ValueError ("KNITRO-Python error: Arrays cType, cLoBnds and cUpBnds have different sizes!")
	if jacIndexVars != None:
		nnzJ = len (jacIndexVars)
	...
	if hessIndexCols != None and nnzH != len (hessIndexCols):
		raise ValueError ("KNITRO-Python error: Arrays hessIndexRows and hessIndexCols have different sizes!")
	if lambdaInitial != None and len (lambdaInitial) > 0 and  n+m != len (lambdaInitial):
		raise ValueError ("KNITRO-Python error: Array lambdaInitial has invalid non-zero size (must be exactly n+m)!")
...
def KTR_solve (
	...
	'''
	if c != None and len (c) > 0 and len (x) + len (c) != len (lambda_):
		raise ValueError ("KNITRO-Python error: Arrays x, c and lambda_ have incomptatible sizes!")
	if objGrad != None and len (objGrad) > 0 and len (x) != len (objGrad):
		raise ValueError ("KNITRO-Python error: Arrays x and objGrad have incomptatible sizes!")
	if hessVector != None and len (hessVector) > 0 and len (x) != len (hessVector):
...
def KTR_set_findiff_relstepsizes (kc, relStepSizes):
	return _knitro.KTR_set_findiff_relstepsizes (kc, relStepSizes)
...
KTR_PARAM_BAR_MAXBACKTRACK		= 1044
...
KTR_PARAM_PAR_BLASNUMTHREADS	= 3004
